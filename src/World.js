...
export class World {
    ...
    setupGround() {
        const ground = new THREE.Mesh(new THREE.PlaneGeometry(200, 200), new THREE.MeshStandardMaterial({ color: 0x1a1a1a, roughness: 1 }));
        ground.rotation.x = -Math.PI/2; ground.position.y = -0.1;
        this.scene.add(ground);
    }

    setupWater() {
        const water = new THREE.Mesh(new THREE.PlaneGeometry(500, 500, 64, 64), new THREE.ShaderMaterial({
            vertexShader: WaterVert, fragmentShader: WaterFrag,
            uniforms: { uTime: { value: 0 }, uColorShallow: { value: new THREE.Color(0x4fc3f7) }, uColorDeep: { value: new THREE.Color(0x0277bd) } },
    ...
    setupTrees() {
        ...
        for (let i = 0; i < 20; i++) {
            ...
            leaves.position.y = 4; t.add(leaves);
